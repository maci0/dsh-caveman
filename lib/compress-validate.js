...
function extractFencedSpans(lines) {
    const spans = [];
    let i = 0;
...
function extractIndentedCodeBlocks(text) {
    const blocks = [];
    ...
    while (i < lines.length) {
        ...
        if (!inList && previousBlank && indent >= 4) {
            ...
            while (i < lines.length && !fenced.has(i)) {
                ...
                if (current.length - current.trimStart().length < 4)
                    break;
                run.push(current);
                i += 1;
            }
            if (run.length > 0)
                blocks.push([start, run.join('\n')]);
            previousBlank = false;
            continue;
        }
        previousBlank = false;
...
export function extractCodeBlocks(text) {
    ...
    while (i < lines.length) {
        ...
        if (closed)
            blocks.push([start, blockLines.join('\n')]);
    }
