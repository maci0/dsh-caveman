...
export function isSensitivePath(filePath) {
    if (SENSITIVE_BASENAME_REGEX.test(basename(filePath)))
        return true;
 ...
 */
export function setBackupRootOverride(dir) {
 ...
 */
export function writeBytesAtomic(filePath, data) {
    ...
    try {
        if (existsSync(filePath))
            chmodSync(tmp, statSync(filePath).mode & 0o777);
 ...
 */
export function writeTextAtomic(filePath, text, newline = '\n') {
...
export function readSource(filePath) {
    ...
    try {
        ...
        throw new Error(`Refusing to compress ${filePath}: not valid UTF-8. `
 ...
 */
export function withFileLock(filePath, run) {
    ...
    if (existsSync(marker)) {
        throw new Error(`Another caveman-compress run appears to be compressing ${filePath}. `
