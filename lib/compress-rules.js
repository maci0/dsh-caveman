...
const FENCE_CLOSE_CACHE = new Map();
/**
 ...
 */
const RENAME = new RegExp(String.raw `\b(?:${[
 ...
 */
const DROP = new RegExp(String.raw `\b(?:${[
...
export function maskCodeBlocks(text) {
    ...
    while (i < lines.length) {
        ...
        if (start > cursor)
            out.push(lines.slice(cursor, start).join('\n'));
...
export function compressLine(line) {
    ...
    let out = masked.replace(RENAME, rewriteReplacer);
    ...
    if (spans.length === 0)
        return out;
    return out.replace(INLINE_SPAN_MARKER, (_, index) => spans[Number(index)] ?? '');
}
/**
