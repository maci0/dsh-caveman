 ...
 */
function loadYaml(): Promise<Yaml> {
  asyncYaml ??= import('yaml') as Promise<Yaml>
  return asyncYaml
}
...
function readDoubleQuoted(text: string): string | undefined {
  let out = ''
  let index = 1
  while (index < text.length) {
    const char = text[index]!
    if (char === '"') {
      if (text.slice(index + 1).trim() !== '') return undefined
      return out
    }
    if (char === '\\') {
      const escape = text[index + 1]
      if (escape === '\\' || escape === '"') out += escape
...
/** Read a `'…'` scalar, or `undefined` when YAML could read the text differently. */
function readSingleQuoted(text: string): string | undefined {
  let out = ''
  let index = 1
  while (index < text.length) {
    const char = text[index]!
...
function readPlain(text: string): string | undefined {
  if (text === '' || NON_STRING.test(text)) return undefined
  if (text.includes('\t') || text.includes(': ') || text.endsWith(':')) return undefined
  if (text.includes(' #')) return undefined
  for (const char of '[]{}') if (text.includes(char)) return undefined
  if (INDICATOR.includes(text[0] ?? '')) return undefined
  const first = text[0]
  if ((first === '-' || first === '?' || first === ':') && (text.length === 1 || text[1] === ' ')) return undefined
  return text
}

/** Read one inline value: block-scalar header, quoted scalar, or plain scalar. */
function readInline(text: string, lines: readonly string[], index: number): { value: unknown; next: number } | undefined {
  const header = /^([|>])([+-]?)$/.exec(text)
  if (header !== null) return readBlockScalar(lines, index, header[1] === '>', header[2] ?? '')
  if (text.startsWith('"')) {
...
function foldLines(core: readonly string[]): string {
  let out = ''
  let blanks = 0
...
function readBlockScalar(
  ...
  for (; index < lines.length; index += 1) {
    ...
    if (line.includes('\t') || /[ ]$/.test(line)) return undefined
  ...
  let trailing = 0
  while (content.length > trailing && content[content.length - 1 - trailing] === '') trailing += 1
  const core = content.slice(0, content.length - trailing)
  if (core[0] === '') return undefined

  const body = folded ? foldLines(core) : core.join('\n')
  if (core.length === 0) return { value: chomp === '+' ? '\n'.repeat(trailing) : '', next: index }
  if (chomp === '-') return { value: body, next: index }
  if (chomp === '+') return { value: body + '\n'.repeat(trailing + 1), next: index }
  return { value: `${body}\n`, next: index }
}
 ...
 */
function parseFlat(block: string): Record<string, unknown> | undefined {
  ...
  for (const line of lines) if (line.includes('\r')) return undefined
  ...
  while (index < lines.length) {
    ...
    if (key === '' || !SIMPLE_KEY.test(key) || NON_STRING.test(key) || !spaced) return undefined
...
function splitFrontmatter(source: string): { raw: string; block: string; body: string; present: boolean } {
  const text = source.replace(/^\uFEFF/, '')
  const match = FRONTMATTER_BLOCK.exec(text)
  if (match === null) return { raw: '', block: '', body: text, present: false }
  const raw = match[0]
...
function fromParsed(raw: string, body: string, parsed: unknown): Frontmatter {
...
export function parseFrontmatter(source: string): Frontmatter {
  ...
  if (!parts.present) return { raw: '', data: {}, body: parts.body }
 ...
 * @returns the verbatim block, the parsed keys, and the remaining body.
 */
export async function parseFrontmatterAsync(source: string): Promise<Frontmatter> {
  const parts = splitFrontmatter(source)
  if (!parts.present) return { raw: '', data: {}, body: parts.body }
  const flat = parseFlat(parts.block)
  if (flat !== undefined) return fromParsed(parts.raw, parts.body, flat)
