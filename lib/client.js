 ...
 *
 * - the Caveman card in Settings → Plugins → Plugin configuration, keyed on the
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      '.dc-card{list-style:none;border:0.5px solid var(--dsw-alias-border-l4);border-radius:16px;background:var(--dsw-alias-bg-layer-3);transition:border-color .16s,background .16s}',
    ...
    const en = {
      ...
      compressEnable: 'Enable /caveman-compress',
      compressHint: 'Rewrites .md/.txt memory files in place; original kept out-of-tree. Off by default.',
      compressBackupDir: 'Backup dir (empty = default)',
      compressBackupHint: 'Where .original.md backups go. Empty uses the platform default.',
    }
    ...
    const zh = {
      ...
      compressEnable: '启用 /caveman-compress',
      compressHint: '就地重写 .md/.txt 记忆文件；原文件在目录外备份。默认关闭。',
      compressBackupDir: '备份目录（空 = 默认）',
      compressBackupHint: '.original.md 备份位置。空则使用平台默认。',
    }
    ...
    function createCard(scope, t) {
      ...
      return function CavemanCard() {
        ...
        const value = snapshot.value !== null && typeof snapshot.value === 'object' ? snapshot.value : {}
        const compressOn = value.compressEnabled === true
        const backupDir = typeof value.compressBackupDir === 'string' ? value.compressBackupDir : ''
        const [draftDir, setDraftDir] = React.useState(null)
        ...
        return React.createElement(
          'li',
          { className: `dc-card${open ? ' dc-card-open' : ''}` },
          React.createElement(
            'button',
            {
              type: 'button',
              className: 'dc-header',
              'aria-expanded': open,
              'aria-label': `${t(open ? 'collapse' : 'expand')}: Caveman`,
              onClick: () => { setOpen(!open) },
            },
            React.createElement(
              'span',
              { className: 'dc-head' },
              React.createElement('span', { className: 'dc-name' }, 'Caveman'),
              React.createElement(
                'span',
                { className: 'dc-desc' },
                t('description', {
            ...
            React.createElement('span', {
              className: open ? 'dc-chevron dc-chevron-open' : 'dc-chevron',
              'aria-hidden': true,
            }),
          ),
          open
            ? React.createElement(
              ...
              React.createElement(
                'div',
                { className: 'dc-row', role: 'radiogroup', 'aria-label': t('compressGroup') },
                React.createElement(
                  'button',
                  {
                    key: 'compress-toggle',
                    type: 'button',
                    role: 'radio',
                    'aria-checked': compressOn,
                    disabled,
                    className: `dc-pill${compressOn ? ' dc-pill-selected' : ''}`,
                    onClick: () => { write(() => scope.set('compressEnabled', !compressOn)) },
                  },
                  t('compressEnable'),
                ),
              ),
              ...
              React.createElement('input', {
                key: 'compress-backup-dir',
                type: 'text',
                className: 'dc-input',
                disabled,
                placeholder: t('compressBackupDir'),
                title: t('compressBackupHint'),
                value: draftDir === null ? backupDir : draftDir,
                onChange: (event) => { setDraftDir(event.target.value) },
                onBlur: () => {
                  if (draftDir !== null && draftDir !== backupDir) {
                    write(() => scope.set('compressBackupDir', draftDir))
                  }
                  setDraftDir(null)
                },
              }),
